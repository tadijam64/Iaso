...
class HealthFirebaseManager {
  ...
  HealthOverview getHealthOverview(List<HealthCheck> healthChecks) {
    ...
    var dryCoughScore = healthChecks.where((entry) => entry.cough == 1).length /
        healthChecks.length *
        100 *
        coughScoreMultipier;
    var productiveCoughScore =
        healthChecks.where((entry) => entry.cough == 2).length /
            healthChecks.length *
            100 *
            coughScoreMultipier;
    var headAcheScore = healthChecks.where((entry) => entry.headache).length /
        healthChecks.length *
        100 *
        headacheScoreMultipier;
    var musclePain = healthChecks.where((entry) => entry.musclePain).length /
        healthChecks.length *
        100 *
        musclePainMultiplier;
